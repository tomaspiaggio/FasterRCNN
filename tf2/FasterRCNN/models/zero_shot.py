...
from . import vgg16
from . import rpn
from embedder import EmbedderNetwork
from . import math_utils


class ZeroShotObjectDetection(tf.keras.Model):
  def __init__(
    self,
    dimensions,
    allow_edge_proposals,
    custom_roi_pool,
    ...
    # TODO(tomaspiaggio) freeze resnet 50 layers so they are not trainable
    #  and add correct loss function. Also see if the output of the resnet is
    #  actually correct.
    self._stage1_feature_extractor = tf.keras.applications.resnet50.ResNet50(
      weights="imagenet",
      include_top=False,
      # Because there's no top, the convolutional layers need not be limited
    ...
    self._stage3_embedder_network = EmbedderNetwork(
  ...
  def call(self, inputs, training = False):
    ...
    if training:
      ...
      proposals, gt_classes, gt_box_deltas = self._label_proposals(
        proposals=proposals,
        # TODO(tomaspiaggio) remove ground truth box ids as they won't be
        #  needed any longer
    ...
    # Stage 3: Detector
    embeddings, embedder_box_deltas = self._stage3_embedder_network(
      inputs=[
        input_image,
        feature_map,
        proposals
    ...
    if training:
      ...
      embedding_loss = self._stage3_embedder_network.embedding_loss(y_predicted=embeddings, y_true=gt_classes, from_logits=not self._activate_class_outputs)
      embedding_box_regressor_loss = self._stage3_embedder_network.regression_loss(y_predicted = embedder_box_deltas, y_true = gt_box_deltas)
      self.add_loss(rpn_class_loss)
      self.add_loss(rpn_regression_loss)
      self.add_loss(embedding_loss)
      self.add_loss(embedding_box_regressor_loss)
      self.add_metric(rpn_class_loss, name="rpn_class_loss")
      self.add_metric(rpn_regression_loss, name="rpn_regression_loss")
      self.add_metric(embedding_loss, name="embedding_loss")
      self.add_metric(embedding_box_regressor_loss, name="embedding_box_regressor_loss")
    else:
      ...
      embedding_box_regressor_loss = float("inf")
